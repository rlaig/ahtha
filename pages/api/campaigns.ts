...
const getCampaigns = (
  ...
  const search = isNotNil(req.query.search) && !Array.isArray(req.query.search) &&req.query.search || undefined
  ...
  const filteredData = isNotNil(search) ? 
    campaignsData.filter(
      (campaign: CampaignData) =>
        campaign.name.toLowerCase().indexOf(search) >=0 || campaign.tags.filter(v => v.toLowerCase().indexOf(search) >=0).length
    )
  : campaignsData
